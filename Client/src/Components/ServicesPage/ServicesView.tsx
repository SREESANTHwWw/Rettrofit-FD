...
const ServicesView = () => {
  const { mainService_id, service_id } = useParams();
  ...
  useEffect(() => {
    axios.get(`${Server}/get-one-service/${service_id}`).then((res) => {
  ...
  useEffect(() => {
    axios
      .get(`${Server}/get-related-service/${mainService_id}/${service_id}`)
      .then((res) => {
        setRelatedServices(res.data.relatedServices)
       
      });
  }, [mainService_id, service_id]);
  ...
  const placeOrder = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    axios
      .post(`${Server}/place-service-order`, {
        servicename,
        name,
        number,
        email,
            ...
            <div className="flex">
              ...
              <div className="flex-1 flex justify-center">
                <img
                  ...
                  src={selectedImage || item.ServicesImages[0]}
        ...
        <div className="fixed inset-0 bg-gray-800/40 backdrop-blur-sm flex justify-center items-center z-40 px-4">
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-3xl relative">
            ...
            <div className="w-full flex justify-center mb-6">
              <h1 className="text-2xl md:text-3xl text-teal-600 font-semibold">
                Order Details
              </h1>
            </div>
            ...
            <form className="space-y-4" onSubmit={placeOrder}>
              ...
              <button
                type="submit"
                className="w-full mt-4 p-3 rounded-lg bg-teal-500 hover:bg-teal-600 text-white font-medium transition duration-200"
              >
                Place Order
              </button>
            </form>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4">
    {relatedServices.map((item: any) => (
      ...
      >
        <Link to={`/services/${mainService_id}/${item._id}`}>
        
        <img 
          src={item.ServicesImages[0]} 
          alt={item.servicename} 
