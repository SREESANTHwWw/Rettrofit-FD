...
const Subcategory = () => {
  ...
  const [subcategory, setsubcategoryid] = useState<string>(id || "")




   const fethcategory = ()=>{
    axios.get(`${Server}/get-one-category/${id}`).then((res)=>{
      const subcategories = res.data.categoryget
      const filterdata = subcategories.filter(
        (item:any) => item.subcategory === id
      );
...
console.log(id);
  ...
  return (
    ...
    <div className="flex justify-between items-center mb-4">
      ...
      <button
        className="flex items-center gap-1 bg-teal-500 cursor-pointer text-white px-3 py-1 rounded-md"
                  ...
                  <div className="flex gap-2 flex-wrap md:flex-nowrap">
                    ...
                    {item.hasSubcategory === true ? (
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-white bg-teal-500 rounded cursor-pointer hover:bg-teal-600 transition-colors flex items-center gap-1"
                      >
                       View Product
                      </button>
                    )}  
